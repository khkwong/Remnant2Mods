...
**Investigated a possible pure-Lua workaround, 2026-07-30 — dead end, confirmed (research doc §3.18).** `RegisterHook` is a genuinely different mechanism from delegate binding (hooks a `UFunction` call directly, doesn't touch `MulticastInlineDelegateProperty` at all), so the theory was: hook `UserWidget:OnMouseButtonDown` on a small wrapper widget around each row, skip `Button`/`OnClicked` entirely. Registered cleanly but never fired once — tested against our own widget, real interactive menu buttons, and blank non-interactive menu background, all silent — so click detection is out via this route too, not just via delegates. **The other half — actually opening a browser tab — does work**: `UKismetSystemLibrary::LaunchURL(FString)`, user-confirmed opening a real tab. So the remaining piece for a "clickable" link is purely the *trigger* — keyboard-driven row selection + Enter (already the planned fallback below) can call the same proven `LaunchURL` call; only true mouse point-and-click is off the table in pure Lua.

**MVP shipped, 2026-07-28** (research doc §3.17): the Lua-only panel described above is built and working in `InventoryTracker/Scripts/main.lua`. **F8** opens/closes it (real `MASTER_LIST` data, not test rows); **Page Up**/**Page Down** cycle through all 8 categories (Ring, Amulet, Armor, Weapon, Mod, Mutator, Engram, Trait); a header line shows the current category, position, and owned/total count. Panel covers the full screen (widened from an initial fixed 900×500 box per user feedback — felt cramped once real data was in it). Rows: `[X]`/`[ ]` + item name (green/red) + a plain-text wiki link (`remnant2.wiki.gg/wiki/<Name>`, derived from the name, not clickable — see the blocker above). Traits show their level (`X/10`) when found, or the wiki source text next to the link when missing. Ownership is rescanned fresh every time the panel opens, never cached across opens/closes.
...
**Why Blueprint modding actually fixes the clickability problem (not just "probably"):** both pure-Lua blockers found this session are specifically about doing the binding/dispatch *at runtime, through UE4SS's Lua reflection bridge*. In a real `WidgetBlueprint` asset, wiring a `Button`'s `OnClicked` (or any event) to a `Launch URL` node happens visually in the editor and gets compiled into real bytecode *at author time* — Lua's reflection bridge, and its gaps, are never involved at all. Same reasoning explains why the native-call-skipping issue (research doc §3.18's best explanation for the `RegisterHook` dead end — the engine skips calling a `BlueprintImplementableEvent` when nothing has bytecode wired to it) also stops applying: a Blueprint graph we author ourselves *is* the bytecode.

**Packaging research, 2026-07-30 (desk research only, nothing prototyped):** the current dev environment already has a Blueprint-mod loading mechanism installed and running — `BPModLoaderMod` (its startup log line, `[BPModLoaderMod] Mods/BPModLoaderMod/load_order.txt not present or no matching mods, loading all BP mods in random order`, showed up in this session's own UE4SS log, unprompted). Per [UE4SS's own docs](https://docs.ue4ss.com/feature-overview/blueprint-modloader.html) and community sources, `BPModLoaderMod` looks for `.pak` files in `Content/Paks/LogicMods`, matched to a same-named subfolder, and specifically expects an asset named exactly `ModActor` — it spawns that as a world `Actor` and runs its `PreBeginPlay`/`PostBeginPlay` lifecycle hooks. That's a **gameplay-logic-mod pattern**, not a plain-UI-asset pattern — it doesn't map directly onto "package one custom `WidgetBlueprint` class and let Lua instantiate it on demand," which is what this mod would actually need. Two live open questions for whenever this gets prototyped, neither answerable without hands-on testing:
- Does simply getting our custom classes *mounted* via a `LogicMods` pak (ignoring `BPModLoaderMod`'s own `ModActor` auto-spawn convenience) make them reachable by Lua the same way base-game classes already are (`StaticFindObject`, `WidgetBlueprintLibrary.Create`) — or does something about `BPModLoaderMod`'s loading path prevent that?
- `Allow Asset Mods` (AAM, already installed in this dev environment, research doc §4.1) is a separate, more general asset-mounting tool — might be the more direct fit for "just get one custom Blueprint asset visible to the engine" than `BPModLoaderMod`'s actor-spawn-oriented design. Untested which (or both) actually works for this use case.

None of this changes the plan — still deferred until the user is ready to invest in learning Blueprint/UE project work — but now the packaging side has a concrete starting point (a loader already installed) instead of being a total unknown.
