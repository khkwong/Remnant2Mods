...
## The in-game menu & tooltip widget family (mod #2 / LoadoutNamer session, 2026-07-13)

Dumps in `dev-data/`: `Widget_InGameMenu.json`, `Widget_TabButton.json`, `Widget_LoadoutTooltip.json`, `Widget_Loadout_Tooltip_Section.json`, `Widget_Tooltip_Actions.json`, `Widget_KeyIcon.json`. Techniques built on these live in `remnant2-modding-research.md` §3.4aa/3.4bb; mod-specific usage in `docs/loadout_namer.md`.

- **`Widget_InGameMenu_C`** (`/Game/UI/UI_InGameMenu/Widget_InGameMenu`) — the whole in-game menu shell incl. the top tab bar. Tabs are `Widget_TabButton_C` instances in a `TabHost` HorizontalBox, named `CharacterTab`, `InventoryTab`, `MapTab`, `TraitTab`, `FragmentsTab`, `ArchetypeTab`, `SystemTab`, `BuyTab`, `QuestTab` (per-instance overrides set only `Label` + `TabPanel`). Public BP functions `FocusTraits/FocusInventory/FocusMap/FocusCharacter/...` are the keyboard-hotkey dispatch targets — the key→function mapping itself is native (nowhere in any widget asset). Full keyboard hotkey map (user-tested): T Traits, I Inventory, M Map, U Fragments, J Character, O System, P Archetype, B close menu, Q/E scroll tabs. The `Focus*` functions early-out on the target tab button's `IsVisible()` — hiding a tab button (visibility Hidden/Collapsed) disables its hotkey; `SetIsEnabled(false)` does NOT. Every arrival at the character tab reconstructs the loadouts panel.
- **`Widget_TabButton_C`** (`/Game/UI/UI_Widgets/Widget_TabButton`) — thin visual tab button (Label, hover anim, `OnTabPressed` delegate), extends native `FocusWidget` (`/Script/GunfireRuntime`). No hotkey data.
- **`Widget_LoadoutTooltip_C`** (`/Game/UI2/.../UI_Game_Character/Widget_LoadoutTooltip`) — the loadout-tile hover tooltip, extends native `FocusTooltipWidget`. Named variables: `ItemLabel` (title TextBlock, static text "Loadout" — never varied at runtime, safe to overwrite), `EmptySlotText`, per-gear `Widget_Loadout_Tooltip_Section` instances, and `Widget_Tooltip_Actions` (the action footer). Multiple instances can be alive simultaneously (one lingering per recently-hovered tile).
- **`Widget_Tooltip_Actions_C`** (`/Game/UI2/.../UI_Game_Tooltips/Widget_Tooltip_Actions`) — the shared action-prompt footer used by item and loadout tooltips. Driven by an `InputActions` TMap<`E_TooltipContextAction`, FText> rebuilt by `Build_List`; per-action buttons are `Widget_ButtonEx_C`. Has an **`ExtraActionList` HorizontalBox — a purpose-built extension point for extra entries** (used by LoadoutNamer for its "F2 Rename" prompt).
- **`Widget_KeyIcon_C`** (`/Game/UI/UI_Widgets/Widget_KeyIcon`) — the reusable key-glyph badge (Background image + `KeyText` TextBlock). Has an `InputActionName` FName property for auto-resolving the bound key, plus `SetActionText(String)`/`ModifyKeyName(String)`. Writing `KeyText` directly with `SetText` works for arbitrary key labels. Live instances are also registered inside `RemnantImageSet_PC` ImageID entries — the game's inline-image set for rich text key glyphs.
- **Fonts**: the UI's cracked display font is `/Game/UI/Fonts/GFGRemnantCracked_Font.GFGRemnantCracked_Font` (tile titles size 12 color 0.7/0.7/0.7, subtitles size 10).
