...
- **New install layout**: everything lives in `Win64\ue4ss\` now (`UE4SS.dll`, `UE4SS-settings.ini`, `Mods\`, `UE4SS.log`), with only the `dwmapi.dll` proxy in `Win64\`. Mod registry is BOTH `mods.txt` and a new `mods.json` — keep them in sync. Symlink script updated accordingly (`scripts\New-ModSymlink.ps1` now defaults to the `ue4ss` subfolder).
...
- Still unproven for the namer input UX: commit detection (Enter → `OnTextCommitted` delegate vs. polling), pre-filling the box (`SetText` — a reflected function call with an FText arg, riskier class per 3.4b), positioning the box anywhere other than as a LoadoutList child, and gamepad interaction.
