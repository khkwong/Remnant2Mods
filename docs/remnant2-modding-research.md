...
**Known accepted limitation, stable-only, not from today's fix**: the pre-existing first-launch `NotifyOnNewObject` quirk (§3.4k/3.4t, the reason we upgraded to experimental in the first place) still reproduces on stable — the very first Loadouts-screen-open after a cold launch gets no extra tiles; closing and reopening the screen once fixes it for the rest of the session. A hot-reload (Ctrl+R) also works as a one-time per-launch workaround. This is unrelated to and not fixed by the capability-gating work above.
